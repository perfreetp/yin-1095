...
import type { CareProgram } from '../../shared/types'
...
export default function CareChannel() {
  ...
  const [programs, setPrograms] = useState<(CareProgram & { applied?: boolean; applyStatus?: string | null })[]>([])
      ...
      try {
        ...
        if (json.success) {
          ...
          const applied = json.data
            .filter((p: CareProgram & { applied?: boolean }) => p.applied)
            .map((p: CareProgram & { applyStatus?: string | null }) => ({
              programId: p.id,
              status: p.applyStatus || 'pending',
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (json.success) {
        ...
        setPrograms((prev) =>
          prev.map((p) => (p.id === selectedProgram.id ? { ...p, applied: true, applyStatus: 'pending' } : p)),
        )
      } else {
  ...
  return (
    <div className="space-y-8">
      ...
      {appliedPrograms.length > 0 && (
        <div className="stagger-enter relative overflow-hidden rounded-3xl p-6 bg-gradient-to-br from-sage-50 via-warm-50 to-rose-50 border border-sage-200/60">
          <div className="absolute -top-10 -right-10 w-48 h-48 rounded-full bg-sage-200/40 blur-3xl" />
          <div className="relative z-10 flex items-start gap-4">
            <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-sage-400 to-sage-500 flex items-center justify-center shadow-lg shadow-sage-200/60 shrink-0">
              <CheckCircle2 className="w-6 h-6 text-white" strokeWidth={2} />
            </div>
            <div className="flex-1 min-w-0">
              <h3 className="text-lg font-bold text-clay-800 mb-2">我的报名状态</h3>
              <div className="space-y-2">
                {appliedPrograms.map((ap) => {
                  const program = programs.find((p) => p.id === ap.programId)
                  if (!program) return null
                  const statusMap: Record<string, { label: string; color: string }> = {
                    pending: { label: '审核中', color: 'bg-warm-100 text-warm-600 border-warm-200' },
                    approved: { label: '已通过', color: 'bg-sage-100 text-sage-600 border-sage-200' },
                    rejected: { label: '未通过', color: 'bg-rose-100 text-rose-600 border-rose-200' },
                  }
                  const status = statusMap[ap.status] || statusMap.pending
                  return (
                    <div
                      key={ap.programId}
                      className="flex flex-wrap items-center gap-3 p-3 rounded-2xl bg-white/70 backdrop-blur-sm border border-white"
                    >
                      <span className="font-medium text-clay-700 text-sm">{program.title}</span>
                      <span
                        className={cn(
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
          {processSteps.map((step, idx) => {
            ...
            return (
              ...
              >
                <div className="absolute top-4 right-4 w-12 h-12 rounded-2xl bg-white/60 backdrop-blur-sm flex items-center justify-center text-2xl font-bold text-clay-300">
                  {step.step}
                </div>
                <div
                  className={cn(
                ...
                <h3 className={cn('text-lg font-bold mb-2', step.text)}>{step.title}</h3>
        ...
        {loading ? (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            {Array.from({ length: 4 }).map((_, i) => (
              <div
                key={i}
                className="h-72 rounded-3xl bg-clay-50 animate-pulse stagger-enter"
              ...
              return (
                ...
                >
                  ...
                  <div className="relative z-10 p-6">
                    ...
                    <div className="mb-5 p-3 rounded-2xl bg-clay-50/60 border border-clay-100">
                      <p className="text-xs font-semibold text-clay-700 mb-1.5">📋 资格标准</p>
                      <ul className="space-y-1">
                        {program.eligibilityCriteria.slice(0, 2).map((c, i) => (
                          <li key={i} className="text-xs text-clay-600 flex items-start gap-1.5">
                    ...
                    <div className="flex items-start gap-2 mb-4 p-3 rounded-2xl bg-warm-50/60 border border-warm-100">
                      <Lock className="w-4 h-4 text-warm-500 shrink-0 mt-0.5" strokeWidth={1.8} />
                  ...
                  <div className="space-y-4 pt-2">
                    <div>
                      ...
                      <select
                        ...
                        className="w-full px-4 py-3 rounded-2xl border border-warm-200 bg-white text-sm text-clay-700 placeholder-clay-400 focus:outline-none focus:border-rose-300 focus:ring-4 focus:ring-rose-100/60 transition-all"
                      >
                        <option value="">请选择方便联系的时间段</option>
                        <option value="morning">工作日上午（09:00-12:00）</option>
                        <option value="afternoon">工作日下午（14:00-18:00）</option>
                        <option value="evening">工作日晚间（19:00-21:00）</option>
                        <option value="weekend">周末全天</option>
                        <option value="anytime">时间灵活，均可安排</option>
                      </select>
                    </div>
                    ...
                    <div>
                      ...
                      <input
                        ...
                        placeholder="如：企业微信、手机号、邮箱等，便于专员与您联系"
                        className="w-full px-4 py-3 rounded-2xl border border-warm-200 bg-white text-sm text-clay-700 placeholder-clay-400 focus:outline-none focus:border-rose-300 focus:ring-4 focus:ring-rose-100/60 transition-all"
                      />
                    </div>
                  ...
                  <div className="space-y-2.5">
                    <div className="flex items-start gap-2.5">
                      ...
                      <p className="text-sm text-clay-600">
                        健康服务专员将在<span className="font-semibold text-sage-600">3个工作日内</span>审核您的报名
                      </p>
                    </div>
                    <div className="flex items-start gap-2.5">
